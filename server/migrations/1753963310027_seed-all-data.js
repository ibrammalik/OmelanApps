...
exports.up = (pgm) => {
  ...
  const appointments = seedAppointments(clients, partners, schedules, now);
  ...
  pgm.sql(`
    ...
    VALUES ${formatValues(clients, ['id', 'username', 'fullname', 'password', 'age', 'address', 'biodata', 'photo_url', 'created_at', 'updated_at'])};
  `);
  ...
  pgm.sql(`
    ...
    VALUES ${formatValues(partners, ['id', 'username', 'fullname', 'password', 'age', 'address', 'biodata', 'photo_url', 'average_rating', 'created_at', 'updated_at'])};
  `);
  ...
  VALUES ${formatValues(schedules, ['id', 'user_id', 'date_start', 'date_end'])};
`);
  ...
  pgm.sql(`
    ...
    VALUES ${formatValues(appointments, ['user_client_id', 'user_partner_id', 'schedule_appointment_id', 'status', 'duration', 'cost_estimation', 'created_at', 'updated_at'])};
  `);

  //reviews
  pgm.sql(`
    INSERT INTO reviews (appointment_id, user_client_id, user_partner_id, rating, comment, created_at, updated_at)
    VALUES ${formatValues(reviews, ['appointment_id', 'user_client_id', 'user_partner_id', 'rating', 'comment', 'created_at', 'updated_at',])};
  `);
};
